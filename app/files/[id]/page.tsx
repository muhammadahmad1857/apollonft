...
export default function FileDetailPage() {
  ...
  useEffect(() => {
    ...
    const fetchFile = async () => {
      ...
      try {
        ...
        // Check if file belongs to connected wallet
        if (data[0].wallet_id.toLowerCase() !== address.toLowerCase()) {
        ...
        // If it's a JSON file, fetch and parse metadata
        if (data[0].type === ".json") {
          try {
            const httpUrl = data[0].ipfsUrl.replace(
              "ipfs://",
  ...
  const isMediaFile = (type: string) => {
    return [".mp3", ".mp4", ".wav"].includes(type);
  };
        ...
        >
                ...
                {metadata.media && (
                  <div>
                    ...
                    <div className="rounded-lg border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900 p-4">
                      {metadata.media.includes(".mp4") ? (
                        <video
                          controls
                          className="w-full rounded-lg"
                          src={getIpfsGatewayUrl(metadata.media)}
                        >
          ...
          >
            ...
            <div className="space-y-4">
              ...
              {file.type === ".json" && !metadata && (
                <div className="rounded-lg border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900 p-4 text-center">
                  <FileText className="h-8 w-8 mx-auto text-zinc-400 mb-2" />
                  <p className="text-zinc-600 dark:text-zinc-400 text-sm">
                    Loading metadata or metadata unavailable
                  </p>
                </div>
              )}
